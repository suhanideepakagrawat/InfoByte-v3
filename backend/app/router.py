...
from app.retrievers.nutrition import retrieve_nutrition
...
def _run_medical_research(query: str):
    return asyncio.run(
        handle_medical_research(query)
    )


def _run_medicine(query: str):
    return asyncio.run(
        handle_medicine_query(query)
    )


def _run_nutrition(query: str):
    return asyncio.run(
...
def _inject_intent(
    ...
    if (
        "error" in response
        and
        "display_payload" not in response
    ):
...
def route_query(
    ...
    # ------------------------------------------------------
    # 3. Execute Retrievers in Parallel
    # ------------------------------------------------------
...
def fetch_selected_wikipedia_article(
    article_url: str
) -> dict:
